...
export default function StrategyDetailPage({ data, setDetail }) {
  const { projectId } = useParams();
  const strategyProjectId = String(projectId || '');
  const unlinkedCompany = strategyProjectId.startsWith('unlinked-') ? strategyProjectId.replace('unlinked-', '') : '';
  const unlinkedProjects = unlinkedCompany
    ? data.projects.filter((item) => !item.goalId && !(item.tags || []).some((tag) => String(tag).toLowerCase() === 'цель') && (item.tags || []).some((tag) => String(tag).toLowerCase() === unlinkedCompany))
    : [];
  const project = data.projects.find((item) => item.id === strategyProjectId) || (unlinkedCompany ? {
    id: strategyProjectId,
    name: 'Без цели',
    responsible: 'Не назначен',
    company: unlinkedCompany,
    virtual: true
  } : null);
  const strategyTasks = data.tasks.filter((task) => task.projectId === strategyProjectId);
  const linkedProjects = unlinkedCompany ? unlinkedProjects : data.projects.filter((item) => item.goalId === strategyProjectId);
  const linkedProjectIds = new Set(linkedProjects.map((item) => item.id));
  const linkedTasks = data.tasks.filter((task) => linkedProjectIds.has(task.projectId));
  ...
  if (!project) {
    return <EmptyState title="Цель не найдена" description={`В Bitrix24 не найден проект цели с ID ${strategyProjectId}. Вернитесь к общему списку целей.`} />;
  }

  return (
    <>
      <Link className="btn mb-4 inline-flex" to="/strategy">← Все цели</Link>
      <StrategyHero project={project} model={model} />

      {!project.virtual && <section className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
      ...
      {!project.virtual && <section className="mb-4 grid grid-cols-1 gap-4 xl:grid-cols-3">
      ...
      <GoalProjectsDashboard projects={linkedProjects} tasks={linkedTasks} setDetail={setDetail} />

      {!project.virtual && <section className="panel mb-4 p-4" onClick={() => setSelectedDirectionId('')}>
      ...
      {!project.virtual && <RelationshipMap directions={model.directions} orphans={model.orphans} allTasks={data.tasks} relationMeta={data.meta.taskRelations} />}

      {!project.virtual && <Gantt projects={[project]} tasks={strategyTasks} selectedProject={project} onClearSelected={() => setSelectedDirectionId('')} hierarchyMode />}

      {!project.virtual && <DataTable
...
function GoalProjectsDashboard({ projects, tasks, setDetail }) {
  if (!projects.length) return (
    <section className="panel mb-4 p-4">
      <h2 className="text-lg font-bold">Проекты цели</h2>
      <p className="mt-1 text-sm text-slate-500">К этой цели пока не прикреплены проекты.</p>
    </section>
  );
  ...
  return (
    <>
      <section className="panel mb-4 p-4">
        <div className="mb-4"><h2 className="text-lg font-bold">Проекты, реализующие цель</h2><p className="mt-1 text-sm text-slate-500">Операционные проекты, их выполнение, сроки и финансовый вклад в достижение цели.</p></div>
        <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <Metric title="Проектов" value={projects.length} onClick={() => openProjects('Проекты цели')} />
          <Metric title="Выполнение задач" value={`${completion}%`} onClick={() => openProjects('Выполнение проектов')} />
          <Metric title="Доход проектов" value={income.toLocaleString('ru-RU')} onClick={() => openProjects('Доход проектов')} />
          <Metric title="Прибыль проектов" value={profit.toLocaleString('ru-RU')} onClick={() => openProjects('Прибыль проектов')} />
        </div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{projects.map((item) => <ProjectCard key={item.id} project={item} onClick={() => openProject(item)} />)}</div>
      </section>
      <section className="mb-4 grid grid-cols-1 gap-4 xl:grid-cols-2">
        <ChartCard title="Финансовый вклад проектов"><FinanceBars data={projects.map((item) => ({ name: item.name, income: item.income || 0, expense: item.expense || 0, profit: item.profit || 0 }))} /></ChartCard>
        <ChartCard title="Прогресс проектов"><BarSimple data={projects.map((item) => ({ name: item.name, value: Math.round((item.closedTasks / Math.max(item.taskCount, 1)) * 100) }))} /></ChartCard>
      </section>
      <Gantt projects={projects} tasks={tasks} onSelectProject={openProject} onClearSelected={() => {}} />
    </>
  );
}
